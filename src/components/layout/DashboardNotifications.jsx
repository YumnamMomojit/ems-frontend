 ...
 * Role-aware notification popover that fetches notifications from generic API
 * 
 * @param {function} onClose - Callback when popover is closed
 */
const DashboardNotifications = ({ onClose }) => {
    ...
    // Determine "View all" link based on user role
    const getViewAllLink = () => {
        const role = user?.role?.toUpperCase();
        if (role === 'HR') return '/hr/settings/notifications';
        if (role === 'ADMIN' || role === 'SUPER_ADMIN' || role === 'ORG_ADMIN') return '/admin/profile';
                ...
                <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                    {loading ? (
                        ...
                        <div className="p-8 text-center text-muted-foreground">
                            <span className="material-symbols-outlined text-[48px] text-muted-foreground/50 mb-2">
                                notifications_off
                            </span>
